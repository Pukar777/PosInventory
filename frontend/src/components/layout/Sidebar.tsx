...
import {
    LayoutDashboard, Package, Beaker, MenuSquare,
...
export function Sidebar({ isMobileMenuOpen, closeMobileMenu }: SidebarProps) {
    ...
    const { criticalCount, lowCount } = useInventoryStore();
    ...
    return (
        <aside className={`fixed inset-y-0 left-0 w-64 flex flex-col border-r border-border/50 bg-card/95 md:bg-card/50 backdrop-blur-xl z-30 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
            <div className="h-16 flex justify-between items-center px-6 border-b border-border/50 shrink-0">
                <div>
                    <span
                        className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-amber-200 drop-shadow-md"
                    ...
                    {({ isActive }) => (
                        <>
                            ...
                            {(criticalCount > 0 || lowCount > 0) && (
                                <Badge
                                    className={`ml-auto text-[10px] h-5 px-1.5 font-mono rounded-full leading-none flex items-center justify-center ${criticalCount > 0 ? 'bg-red-500 text-white hover:bg-red-600 border-transparent shadow-[0_0_6px_rgba(239,68,68,0.4)]' : 'bg-amber-500 text-amber-950 hover:bg-amber-600 border-transparent shadow-[0_0_6px_rgba(245,158,11,0.4)]'}`}
                                >
