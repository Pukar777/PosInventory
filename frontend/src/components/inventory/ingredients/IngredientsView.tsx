...
export function IngredientsView() {
    ...
    const fetchIngredients = async () => {
        setIsLoading(true);
        try {
            const response = await request({ url: '/ingredients', method: 'GET' });
            if (response.data.success) {
                // Map the backend data to calculate status
                const mappedIngredients = response.data.data.map((ing: any) => {
                    ...
                    if (stockNum <= minNum) status = 'low';
                    else if (stockNum <= minNum * 1.5) status = 'warn';
